  ...
  switch (nivel) {
    case 'owner':
      return [
        ...
        { href: '/dashboard/owner/usuarios',     label: 'Usuários',         icon: icon(Users) },
...
export default function Sidebar({ nivel, open, onClose, isDesktop }: Props) {
  ...
  return (
    <nav style={sidebarStyle} aria-label="Menu principal">
      ...
      {/* Itens do menu */}
      {items.map(item => {
        const active = pathname === item.href || (item.href !== '/dashboard/' + nivel && pathname.startsWith(item.href + '/'))
        return (
          <Link
            key={item.href}
            href={item.href}
      ...
      {/* Rodapé da sidebar */}
      <div style={{ marginTop: 'auto', padding: '16px 20px 8px', borderTop: '1px solid #f1f5f9', marginTop: '24px' }}>
